...
/**
 * Server-side utility to get project profitability data
 * Replaces server action for API route usage
 */
export async function getProjectProfitabilityDataServer(
    businessId: string,
    filters?: {
        status?: string;
        clientId?: string;
    ...
    try {
        // Get projects with client data and aggregated costs
        ...
        const projects: ProjectProfitabilityData[] = (projectsData || []).map((project: any) => {
            ...
            const projectMaterialCosts = (materialCosts || [])
                ...
                .reduce((sum: number, material: any) => sum + ((material.quantity || 0) * (material.cost || 0)), 0);

            // Estimate equipment costs ($100 per usage day)
            ...
            const costOverrun = Math.max(0, currentSpend - budget);
...
/**
 * Server-side utility to get project profitability trends
 * Replaces server action for API route usage
 */
export async function getProjectProfitabilityTrendsServer(
    businessId: string,
    projectId?: string,
    days: number = 30
): Promise<{
