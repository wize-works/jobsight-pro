...
export async function triggerProjectNotification(
    ...
    try {
        const { user } = await withBusinessServer();
        ...
        if (assignedUserIds && assignedUserIds.length > 0) {
            ...
            await sendPushNotificationToBusiness(title, body, { projectId }, url, user.id);
        }
...
export async function triggerTaskNotification(
    ...
    try {
        ...
        if (assignedUserId) {
            ...
            // Notify entire business
            await sendPushNotificationToBusiness(title, body, { taskId }, url, user.id);
        }
...
export async function triggerEquipmentNotification(
    ...
    try {
        ...
        if (assignedUserId) {
            ...
            // Notify entire business
            await sendPushNotificationToBusiness(title, body, { equipmentId }, url, user.id);
        }
...
export async function triggerInvoiceNotification(
    ...
    try {
        ...
        const excludeUserId = action === 'created' ? user.id : undefined;
...
export async function triggerSystemNotification(
    ...
    try {
        ...
        // Notify entire business
        await sendPushNotificationToBusiness(title, message, {}, url, user.id);
