...
// Lazy load the crew members modal
const ModalCrewMembers = dynamic(() => import("./modal-crew-members"), {
    loading: () => <ModalLoading message="Loading crew members..." />,
    ssr: false
});
...
export const CrewCard = ({
    ...
    const getCrewInitials = (name: string) => {
        ...
        <>
            ...
            {/* Crew Members Modal */}
            {showCrewMembersModal && (
                <ModalCrewMembers
                    crew={crew}
                    isOpen={showCrewMembersModal}
                    onClose={() => setShowCrewMembersModal(false)}
                    onRefresh={onRefresh}
                />
            )}
        </>);
