...
export async function getCurrentSubscription(): Promise<BusinessSubscription | null> {
    try {
        ...
        const { data, error } = await fetchByBusiness('business_subscriptions', business.id, "*", {
...
export async function createSubscription(
    ...
    try {
        ...
        if (existingSubscription) {
            ...
            const { data, error } = await updateWithBusinessCheck('business_subscriptions', existingSubscription.id,
                {
                    plan_id: planId,
