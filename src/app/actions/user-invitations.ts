...
export async function sendUserInvitation(email: string, name: string, role: string) {
    try {
        ...
        const emailResponse = await resend.emails.send({
            from: process.env.RESEND_FROM_EMAIL || 'JobSight Pro <noreply@pro.jobsight.co>',
            to: email,
        ...
        console.log(`Invitation email sent successfully to ${email}. Message ID: ${emailResponse.data?.id}`);
...
export async function resendUserInvitation(userId: string) {
    try {
        ...
        if (user.status !== 'invited' && user.status !== 'email_failed') {
            throw new Error("Cannot resend invitation to user with current status");
        }
