...
export async function POST(request: NextRequest) {
    try {
        ...
        if (crewError || !crewMember) {
            return NextResponse.json({ success: false, error: 'Crew member not found or access denied' }, { status: 404 });
        }
        ...
        if (error) {
            ...
            return NextResponse.json({ success: false, error: 'Failed to update crew member rate' }, { status: 500 });
        }
