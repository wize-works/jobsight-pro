...
export default function CompactWeatherWidget({
    ...
    useEffect(() => {
        ...
        return (
            <div className={`bg-base-100 rounded-lg p-4 shadow-sm border ${className || ''}`}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    {[...Array(5)].map((_, index) => (
                        <div key={index} className="flex flex-col items-center text-center p-2 rounded-lg bg-base-200/50">
                            <div className="w-8 h-3 bg-base-300 rounded mb-1 animate-pulse"></div>
                            <div className="w-8 h-8 bg-base-300 rounded mb-1 animate-pulse"></div>
                            <div className="w-6 h-3 bg-base-300 rounded mb-1 animate-pulse"></div>
                            <div className="w-6 h-3 bg-base-300 rounded animate-pulse"></div>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <div className={`bg-base-100 rounded-lg p-4 shadow-sm border ${className || ''}`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                {weather.forecast.map((day, index) => (
                    <div
                        key={index}
                        className="flex flex-col items-center text-center p-2 rounded-lg bg-base-200/50 hover:bg-base-200 transition-colors"
                        title={`${day.date}: ${day.condition}`}
                    >
                        <div className="text-xs font-medium text-base-content/70 mb-1">
                            {day.date}
                        </div>
                        <div className="text-2xl mb-1" title={day.condition}>
                            {day.icon}
                        </div>
